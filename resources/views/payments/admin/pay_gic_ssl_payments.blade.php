...
@section('title', 'Pay GIC SSL Payments')
...
$(function() {
  ...
  var $tableSel = $('#pay-gic-ssl-payments');
  $tableSel.dataTable({
    "ordering": false,
     dom: 'Bfrtip',
        buttons: [
...
<div class="container-fluid">
  ...
  <div class="panel">
    ...
    <div class="panel-footer">
      ...
      <table id="pay-gic-ssl-payments" class="table table-striped table-bordered" style="width:100%">
            ...
            <tbody>
              ...
              @foreach($pay_gic_ssl_payments as $pay_gic_ssl_payment)

                    <tr>
                      <td>{{ Carbon\Carbon::parse($pay_gic_ssl_payment->created_at)->format('d-M-y') }}</td>
                      <td>{{ $pay_gic_ssl_payment->payment->userInfo->client_code ?? 'Client Removed' }}</td>
                      <td>{{ $pay_gic_ssl_payment->payment->userInfo->name ?? 'Client Removed' }}</td>
                      <td>{{ $pay_gic_ssl_payment->payment->programInfo->program_name ?? 'N/A' }}</td>
                      <td>{{ $pay_gic_ssl_payment->payment->stepInfo->step_name ?? 'N/A' }}</td>
                      <td>

                        @foreach($pay_gic_ssl_payment->payment->userInfo->getAssignedCounselors as $counselors)
                        ...
                        @foreach($pay_gic_ssl_payment->payment->userInfo->getAssignedRms as $rms)
                      ...
                      <td>{{ strtoupper($pay_gic_ssl_payment->bank_name) }}</td>
                      <td>
                        @if($pay_gic_ssl_payment->online_verified == -1)

                        <a href="#" id="{{ $pay_gic_ssl_payment->id }}" class="label label-success" onclick="verifyOnline(this)">Verify</a>

                        <a href="{{ route('payment.online.dissaproved', $pay_gic_ssl_payment->id) }}" class="label label-danger">Reject</a>
                        @elseif($pay_gic_ssl_payment->online_verified == 1)
                        <p class="text-success text-weight-bold">Verified</p>
                        @elseif($pay_gic_ssl_payment->online_verified == 0)
                        <p class="text-danger text-weight-bold">Unverified</p>
                        @endif
                      </td>

                      <td>{{ number_format($pay_gic_ssl_payment->amount_paid) }}</td>
                      <td>{{ number_format($pay_gic_ssl_payment->amount_received) }}</td>

                      <td>

                        <button class="btn btn-info button2 btn-sm" onclick="editOnline(this)" id="{{ $pay_gic_ssl_payment->id }}">Edit</button>
